...
select tb.routeid, tb.stationname, tb.stationtype, tb.buscount, tb.begindate, tb.linkman, tb.routegrade from (
  select si.stationname, s.*, o.buscount, r.begindate, o.linkman, r.routegrade
  from mcrouteinfogs r
  join mcsegmentinfogs sig on r.routeid = sig.routeid
  join mcrsegmentstationgs s on sig.segmentid = s.segmentid
  join mcstationinfogs si on s.stationid = si.stationid
  join mcorginfogs o on r.orgid = o.orgid
  where r.routeid = 124
  and s.stationtype in (6, 9)
  and r.isactive = 1
) tb



select tb.routeid, tb.subrouteid, tb.stationname, tb.stationtype, tb.buscount, tb.begindate, tb.linkman, tb.routegrade, tb.orgid, tb.orgname, tb.parentorgid from (
  select si.stationname, s.routeid, s.stationtype, sig.subrouteid, o.buscount, r.begindate, o.linkman, r.routegrade, o.orgid, o.orgname, o.parentorgid
  from mcrouteinfogs r
  join mcsegmentinfogs sig on r.routeid = sig.routeid
  join mcrsegmentstationgs s on sig.segmentid = s.segmentid
  join mcstationinfogs si on s.stationid = si.stationid
  join mcorginfogs o on r.orgid = o.orgid
  where s.stationtype in (6, 9)
  and r.isactive = 1 and s.isactive = 1
  and o.parentorgid = 2188
) tb
order by o.routeid, subrouteid
